...
program
	...
	.action(
		async function (
			...
			options['sandboxId'] =
				options.sandboxId || configUtils.searchProperty(SANDBOX_ID);
...
list
	...
	.option('--asc, --ascending', 'Set the sort direction to ascending order')
	.option('--desc, --descending', 'Set the sort direction to descending order')
	.description('List all namespaces')
	.action(async function (environment, options) {
		options['orderBy'] =
			options.orderBy || configUtils.searchProperty(ORDER_BY);
		...
		if (options.ascending && options.descending) {
			cliUtils.logAndExit(
				1,
				'ERROR: Cannot set both ascending and descending sort together.',
			);
		...
		if (options.orderBy) {
			if (!options.details) {
				cliUtils.logAndExit(
					1,
					'ERROR: Cannot use order-by without using detailed list.',
				);
			}
			switch (options.orderBy.toLowerCase()) {
				...
				default:
					cliUtils.logAndExit(
						1,
						`ERROR: Column ${options.orderBy} is not a valid column name.`,
					);
					break;
			}
		} else {
		...
		try {
			await kvCliHandler.listNameSpaces(
				environment,
				options.details,
				sortDirection,
				orderBy,
...
list
	...
	.action(async function (environment, namespace, groupId, options) {
		options['maxItems'] =
			options.maxItems || configUtils.searchProperty(MAX_ITEMS);
		options['sandboxId'] =
			options.sandboxId || configUtils.searchProperty(SANDBOX_ID);
...
list
	...
	.description(
		'List group identifiers created when writing items to a namespace',
	)
	.action(async function (options) {
		options['groupId'] =
			options.groupdId || configUtils.searchProperty(GROUP_ID);
...
create
	...
	.action(async function (environment, namespace, options) {
		options['retention'] =
			options.retention || configUtils.searchProperty(RETENTION);
		options['groupId'] =
			options.groupId || configUtils.searchProperty(GROUP_ID);
		options['geolocation'] =
			options.geolocation || configUtils.searchProperty(GEO_LOCATION);
		options['dataAccessPolicy'] = options.dataAccessPolicy
			? validateNamespaceDataAccessPolicy(options.dataAccessPolicy)
			: undefined;
...
create
	...
	.action(async function (tokenName, options) {
		...
		options['production'] =
			options.production || configUtils.searchProperty(PRODUCTION);
		...
		options['namespace'] =
			options.namespace || configUtils.searchProperty(NAMESPACE);
		options['save_path'] =
			options.save_path || configUtils.searchProperty(SAVE_PATH);
...
const revoke = program.command('revoke').description('Revoke an EdgeKV token');
revoke
	.command('token <tokenName>')
...
const show = program
	.command('show')
	.description(
		'Check the initialization status of the EdgeKV or Retrieve an EdgeKV namespace',
	);
show
	.command('status')
