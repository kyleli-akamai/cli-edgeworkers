...
export function validateNetwork(network: string, sandboxId?: string) {
	if (sandboxId) {
		if (network.toUpperCase() !== cliUtils.staging) {
			cliUtils.logAndExit(
				1,
				`ERROR: Environment parameter must be only staging - was: ${network}`,
			);
		}
	} else {
		if (
			...
			cliUtils.logAndExit(
				1,
				`ERROR: Environment parameter must be either staging or production - was: ${network}`,
			);
		}
	}
}
		...
		dataAccessPolicyArr
			...
			.forEach((item) => {
				...
				if (property.length !== 2) {
					console.error(
						`Warning: cannot parse invalid item ['${item}'], skip it.`,
					);
				} else {
...
export function validateDataAccessPolicy(dataAccessPolicyStr: string) {
	...
	if (
		dataAccessPolicy['restrictDataAccess'] == undefined ||
		dataAccessPolicy['allowNamespacePolicyOverride'] == undefined
	) {
		cliUtils.logAndExit(
			1,
...
export function validateInputFile(itemFilePath) {
	...
	if (!checkIfFileExists(itemFilePath)) {
		cliUtils.logAndExit(
			1,
			`ERROR: Json file provided in the path (${itemFilePath}) is not found or file does not have permissions.`,
		);
	}
...
export function saveTokenToBundle(
	...
	oldTarBallStream.on('error', function () {
		const errMsg =
			'ERROR: Unable to read the tar bundle. Add the token in file edgekv_tokens.js and place it in your tar bundle';
	...
	extract.on('entry', function (header, stream, callback) {
		if (header.name != 'edgekv_tokens.js') {
			// if the folder already has token we don't want to stream it, it creates duplicate header
			stream.pipe(pack.entry(header, callback));
		} else {
			updateFile = true;
			stream.on('data', function (chunk) {
				data += chunk;
				const existingTokenContent = validateAndGetExistingTokenContent(
					data,
					createdToken,
					decodedToken,
					nameSpaceList,
				...
				// updated content
				tokenContent = updateTokenContent(
					existingTokenContent,
					nameSpaceList,
					createdToken,
					decodedToken,
					overWrite,
	...
	extract.on('finish', function () {
		...
		const tokenFileContent = constructTokenFile(tokenContent);
 ...
 * @param nameSpaceList
 */
export function createTokenFileWithoutBundle(
	savePath,
	overWrite,
	createdToken,
	decodedToken,
	...
	fs.writeFile(tokenFilePath, tokenFileContent, function (err) {
		if (err) {
			const errMsg =
				'ERROR: Unable to create a token file. Add the token in file edgekv_tokens.js and place it in your tar bundle';
			if (ekvJsonOutput.isJSONOutputMode()) {
				ekvJsonOutput.writeJSONOutput(
					0,
					msg,
 ...
 * @param nameSpaceList
 */
function validateAndGetExistingTokenContent(
	data,
	createdToken,
	decodedToken,
	nameSpaceList,
	...
	if (
		tokenList.length == 0 ||
		tokenList[0].indexOf('var edgekv_access_tokens') == -1
	) {
		cliUtils.logWithBorder(
			'ERROR : Not a valid EdgeKV Access Token file (missing \'edgekv_access_tokens\' var assignment)!',
		);
		response.logToken(
			createdToken['name'],
			createdToken['value'],
	...
	if (tokenList[1].indexOf('export { edgekv_access_tokens };') == -1) {
		cliUtils.logWithBorder(
			'ERROR : Not a valid EdgeKV Access Token file (missing \'edgekv_access_tokens\' export assignment)!',
		);
		response.logToken(
			createdToken['name'],
			createdToken['value'],
	...
	try {
		...
		cliUtils.logWithBorder(
			`ERROR: Not a valid EdgeKV access token file. Delete or re-create the edgekv token file. ${ex}`,
		);
		response.logToken(
			createdToken['name'],
			createdToken['value'],
			...
			if (tokenName === createdToken['name']) {
				if (overWrite) {
					...
					if (tokenValue != createdToken['value']) {
						cliUtils.logWithBorder(
							`Token value mismatch for token ${tokenName}! Not updating token value. Use '-o' to overwrite token value. Place the below token in edgekv_tokens.js manually`,
						);
					} else {
						cliUtils.logWithBorder(
							`Token value matches for token ${tokenName}. Not updating token value. Use '-o' to overwrite token value.`,
						);
					}
					response.logToken(
						createdToken['name'],
				...
				cliUtils.logWithBorder(
					`ERROR: Token ${createdToken['name']} for namespace ${ns} not found in the file`,
				);
				response.logToken(
					createdToken['name'],
					createdToken['value'],
...
export function convertDaysToSeconds(days: number) {
	if (!isNaN(days) && days >= 0) {
		if (days != 0 && (days < 1 || days > 3650)) {
			...
			cliUtils.logAndExit(
				1,
				'ERROR: A non zero value specified for the retention period cannot be less than 1 day or more than 3650 days.',
			);
		}
		return days * 86400;
	} else {
		cliUtils.logAndExit(
			1,
			'ERROR: Retention period specified is invalid. Please specify the retention in number of days.',
		);
	}
}
