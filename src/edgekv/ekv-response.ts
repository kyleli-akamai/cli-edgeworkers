...
export function logNamespace(nameSpaceId: string, createdNameSpace) {
	const retentionPeriod = ekvhelper.convertRetentionPeriod(
		createdNameSpace['retentionInSeconds'],
	);
	const groupId =
		createdNameSpace['groupId'] == undefined ? 0 : createdNameSpace['groupId'];
	const createNameSpace = {
		Namespace: nameSpaceId,
		RetentionPeriod: retentionPeriod,
		GeoLocation: createdNameSpace['geoLocation'],
		GroupId: groupId,
		'Namespace dataAccessPolicy': createdNameSpace['dataAccessPolicy']
			? 'restrictDataAccess=' +
				createdNameSpace['dataAccessPolicy']['restrictDataAccess'] +
				', policyType=' +
				createdNameSpace['dataAccessPolicy']['policyType']
			: 'N/A',
	};
	console.table([createNameSpace]);
}

export function logAuthGroups(
	retrievedAuthGroups,
	ewGrpCapabilities: Map<number, string>,
	includeEwGroups: boolean,
) {
	if (retrievedAuthGroups['groups']) {
		...
		groups.push({
			GroupId: 'GroupId',
			GroupName: 'GroupName',
			EdgeKVCapabilities: 'EdgeKVCapabilities',
		}); // dirty hack for header
		groups.push({
			GroupId: getBorder,
			GroupName: getBorder,
			EdgeKVCapabilities: getBorder,
		}); // dirty hack for border
		if (includeEwGroups) {
...
function getGroup(
	...
	if (includeEwGroups) {
		groupInfo['EwCapabilities'] = ewGrpCapabilities
			.get(group['groupId'])
			.toString()
			.split(',')
			.join(', '); // need to be formatted for table-layout lib
	}
	return groupInfo;
}
...
export function logInitialize(initializedEdgekv) {
	const initializeStatus = {
		...
		DataAccessPolicy: initializedEdgekv['dataAccessPolicy']
			? 'restrictDataAccess=' +
				initializedEdgekv['dataAccessPolicy']['restrictDataAccess'] +
				', allowNamespacePolicyOverride=' +
				initializedEdgekv['dataAccessPolicy']['allowNamespacePolicyOverride']
			: 'N/A',
	};
...
export function logTokenList(tokenList) {
	...
	tokenList['tokens'].forEach((token) => {
		...
		const tokenContent = {
			...
			ExpiryDate: `${weekday[expiry.getDay()]},${expiry.getDate()} ${
				shortMnthNames[expiry.getMonth()]
			} ${expiry.getFullYear()}`,
			Warning: warning,
		};
...
export function logToken(
	...
	console.log(
		...
		staging +
			'\n' +
			`Issue date:           ${
				weekday[issueDate.getDay()]
			},${issueDate.getDate()} ${
				shortMnthNames[issueDate.getMonth()]
			} ${issueDate.getFullYear()} \n` +
			`Expiry date:          ${
				weekday[expiryDate.getDay()]
			},${expiryDate.getDate()} ${
				shortMnthNames[expiryDate.getMonth()]
	...
	if (difference >= 0 && difference <= 30) {
		console.log(
			`       *** WARNING: Access Token will EXPIRE in less than ${difference} days! ***`,
		);
	} else if (difference <= -1) {
	...
	for (const ns of nameSpaceList) {
		...
		console.log(
			'  ' + ns.substring(ns.indexOf('-') + 1) + ':  [' + permissionList + ']',
		);
	}
}

export const logTokenToJson = (token, decodedToken, nameSpaceList) => ({
	token,
	decodedToken,
	nameSpaceList,
});
