...
export async function listNameSpaces(
	...
	if (nameSpaceList != undefined && !nameSpaceList.isError) {
			...
			namespace.forEach(function (value) {
				if (details) {
					...
					nsListResp.push({
						...
						'Namespace dataAccessPolicy': value['dataAccessPolicy']
							? 'restrictDataAccess=' +
								value['dataAccessPolicy']['restrictDataAccess'] +
								', policyType=' +
								value['dataAccessPolicy']['policyType']
							: 'N/A',
					});
				} else {
		...
		const msg = `The following namespaces are provisioned on the ${environment} environment`;
...
export async function updateDatabase(dataAccessPolicyStr: string) {
	...
	if (
		updateDataAccessPolicy.data != undefined &&
		!updateDataAccessPolicy.isError
	) {
