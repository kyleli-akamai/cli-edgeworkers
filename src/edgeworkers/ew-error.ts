		...
		switch (status) {
			case 404: {
				return {
					isError: true,
					error_reason:
						ErrorMessage[commandId + '_ERROR'] + ' ' + err['detail'],
				};
			}
			case 403: {
				return {
					isError: true,
					error_reason:
						ErrorMessage[commandId + '_ERROR'] + ' ' + ErrorMessage.GENERIC_403,
				};
			}
			case 401: {
			...
			case 504: {
				return {
					isError: true,
					error_reason:
						ErrorMessage[commandId + '_ERROR'] +
						' ' +
						ErrorMessage.EW_TIMEOUT_ERROR,
				};
			}
			default: {
...
export function invalidParameterError(commandId, errDetail = null) {
	return handleError(
		cliUtils.toJsonPretty({ status: 400, detail: errDetail }),
		commandId,
	);
}
