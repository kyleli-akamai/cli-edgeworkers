...
program
  ...
  .option(
    ...
    'Use this section in ew-config file that contains the default config properties set.',
  )
  .option(
    '--json [path]',
...
program
  ...
  .option(
    '-restier, --resourceTierId <resourceTierId>',
    'Filter EdgeWorkers by resource tiers',
  )
  .action(async function (ewId, options) {
    options['groupId'] =
      options.groupId || configUtils.searchProperty(GROUP_ID);
    options['resourceTierId'] =
      options.resourceTierId || configUtils.searchProperty(RESOURCE_TIER_ID);

    try {
      await cliHandler.showEdgeWorkerIdOverview(
        ewId,
        options.groupId,
        options.resourceTierId,
      );
...
program
  ...
  .option(
    '-restier, --resourceTierId <resourceTierId>',
    'New resource Tier ID to associate with EdgeWorker',
  )
    ...
    try {
      ...
      if (!resourceTierId) {
        ...
        if (resourceTierId == undefined) {
          cliUtils.logAndExit(
            1,
            'ERROR: Please select a valid resource tier ID.',
          );
        }
      }
...
program
  ...
  .action(async function (options) {
    options['contractId'] =
      options.contractId || configUtils.searchProperty(CONTRACT_ID);
...
program
  .command(
    'update-id <edgeworker-identifier> <group-identifier> <edgeworker-name>',
  )
  .description(
  ...
  .action(async function (ewId, groupId, name, options) {
    ...
    try {
      await cliHandler.updateEdgeWorkerInfo(
        ewId,
        groupId,
        name,
        options.resourceTierId,
...
program
  ...
  .action(async function (ewId, versionId) {
    try {
      await cliHandler.showEdgeWorkerIdVersionOverview(ewId, {
        versionId: versionId,
        showResult: true,
      });
    } catch (e) {
...
program
  ...
  .action(async function (ewId, options) {
    options['bundlePath'] =
      options.bundlePath || configUtils.searchProperty(BUNDLE_PATH);
    options['workingDirectory'] =
      options.workingDirectory || configUtils.searchProperty(WORKING_DIRECTORY);

    //either bundle or code working directory must be provided or fail
    if (
      (!options.bundle && !options.codeDir) ||
      (options.bundle && options.codeDir)
    )
      cliUtils.logAndExit(
        1,
        'ERROR: You must provide the EdgeWorkers bundle tgz (--bundle) OR the working directory for mainjs and manifest file (--codeDir) to create a new version!',
      );
...
program
  ...
  .action(async function (ewId, versionId, options) {
    options['downloadPath'] =
      options.downloadPath || configUtils.searchProperty(DOWNLOAD_PATH);
...
program
  ...
  .option(
    '--activeOnNetwork',
    'Limits results to show only currently activate versions',
  )
  .option(
    '--network  <network>',
  ...
  .action(async function (ewId, options) {
    ...
    options['activeOnNetwork'] =
      options.activeOnNetwork || configUtils.searchProperty(ACTIVE);
    options['network'] = options.network || configUtils.searchProperty(NETWORK);

    // Do not provide both versionId and activationId
    if (
      options.activationId &&
      (options.versionId || options.active || options.network)
    ) {
      cliUtils.logAndExit(
        1,
        'ERROR: You may not provide the Activation identifier with versionId, network, or activeOnNetwork options.',
      );
    }
...
program
  ...
  .action(async function (ewId, network, versionId) {
    ...
    try {
      await cliHandler.createNewActivation(
        ewId,
        network.toUpperCase(),
        versionId,
      );
...
program
  ...
  .action(async function (ewId, resourceTierId, options) {
    options['ewName'] =
      options.ewName || configUtils.searchProperty(EDGEWORKER_NAME);
    options['groupId'] =
      options.groupId || configUtils.searchProperty(GROUP_ID);

    try {
      await cliHandler.cloneEdgeworker(
        ewId,
        options.groupId,
        options.ewName,
        resourceTierId,
...
program
  ...
  .action(async function (ewId, network, versionId) {
    ...
    try {
      await cliHandler.deactivateEdgeworker(
        ewId,
        network.toUpperCase(),
        versionId,
      );
...
program
  ...
  .option(
    '--expiry <expiry>',
    'The number of minutes during which the token is valid, after which it expires. Max value is 720 minutes(12 hours); default value is 15 minutes.',
  )
  .option(
    '--format <format>',
    'Format in which the output will be printed to console',
  )
...
const get = program
  .command('get')
  .description('Get an EdgeWorkers report or get available report types.');
...
get
  ...
  .requiredOption(
    ...
    'ISO 8601 timestamp indicating the start time of the EdgeWorkers report (REQUIRED).',
  )
  .option(
    '-e, --endDate <endDate>',
    'ISO 8601 timestamp indicating the end time of the EdgeWorkers report. If not specified, the end time defaults to the current time.',
  )
  .option(
    '--status, <status>',
    'Comma-separated string to filter by EdgeWorker status. Values: success, genericError, unknownEdgeWorkerId, unimplementedEventHandler, runtimeError, executionError, timeoutError, resourceLimitHit, cpuTimeoutError, wallTimeoutError, initCpuTimeoutError, initWallTimeoutError.',
  )
  .option(
    '--ev, --eventHandlers <eventHandlers>',
    'Comma-separated string to filter EdgeWorkers by the event that triggers them. Values: onClientRequest, onOriginRequest, onOriginResponse, onClientResponse, responseProvider.',
  )
  .action(async function (reportId: number, edgeworkerId: string, options) {
    ...
    if (!reportId) {
      cliUtils.logAndExit(
        1,
        'ERROR: Please specify a reportId. To obtain the available report ID run "akamai edgeworkers get reports".',
      );
    }

    options['endDate'] =
      options.endDate || configUtils.searchProperty(END_DATE);
    options['status'] = options.status || configUtils.searchProperty(STATUS);
    options['eventHandlers'] =
      options.eventHandlers || configUtils.searchProperty(EVENT_HANDLERS);
...
config
  ...
  .requiredOption(
    ...
    'Save config properties in bulk. Use format \'key=value\' to set a property and white space to split them.',
  )
  .action(async function (options) {
